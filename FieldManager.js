export class FieldManager {
        ...
        if (hasTaskFieldsJSON) {
            ...
            for (let fieldObj of fieldsList) {
                if (fieldObj.type == "checkbox") {
                    this.createCheckbox(pageContainer, fieldObj.x, fieldObj.y, 2, fieldObj.value);
                } else {
            ...
            for (const sheetItem of krSheetItems) {
                const tx = pdfjsLib.Util.transform(viewport.transform, sheetItem.transform);
    ...
    async drawTaskFields(page, hasTaskFieldsJSON, pageContainer) {
        ...
        if (hasTaskFieldsJSON) {
            ...
            let hasNotesField = fieldsList.some(field => field.type == "notes");

            for (let fieldObj of fieldsList) {
                if (fieldObj.type == "notes") {
                    if (fieldObj.value && String(fieldObj.value).trim() != "") {
                    ...
                    if (fieldObj.value.includes("Pakime")) {
                        await this.createSalvestaButton(
                            fieldObj.x - 7,
                            fieldObj.y + 15,
                            pageContainer,
                            this.fetcher
        ...
        salvestaBtn.addEventListener("click", async () => {
            ...
            try {
                ...
                if (!(await this.fetcher.hasTaskFieldsJSON(this.selectedOrder.getGuid()))) {
                    await this.fetcher.createJsonFile(
                        this.selectedOrder.getGuid(),
                        jsonBytes
                    );
                } else {
                    await this.fetcher.replaceJsonFile(
                        this.selectedOrder.getGuid(),
                        await this.fetcher.getOrderFileGUID(
                            this.selectedOrder.getGuid(),
                            "taskFields"
                        ),
                        jsonBytes
    ...
    createNotesField(container, x, y, text, pageNr) {
        ...
        input.placeholder = "Märkmed";

        input.className = "task-field";
        input.dataset.page = pageNr;
        input.dataset.type = "notes";
